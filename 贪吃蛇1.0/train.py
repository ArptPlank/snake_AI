...
class TrainManager():
    ...
    def __init__(self,
        ...
        q_func = modules.DuelingDQN(n_act)
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=factor, patience=patience,threshold=threshold, min_lr=min_lr)
            ...
            with tqdm(total=int(self.episodes / split_num), desc='Iteration %d' % i) as pbar:
                for i_episode in range(int(self.episodes / split_num)):
                    ...
                    if e % 50 == 0:
                        ...
                        plt.draw()
                        plt.pause(0.001)  # 小暂停，使得图形有时间更新-0
                    if e % 1000 == 0:
                        model_state_dict = self.agent.pred_func.state_dict()
                        # 选择保存的文件路径和文件名
                        file_path = "./model/trained_model_" + str(e) + (".pth")
                    ...
                    if e % 50 == 0:
                        ...
                        pbar.set_postfix({'episode': '%d' % e, 'reward': '%.3f' % ep_reward,'length': '%.6f' % avr_length, 'step_num': '%.3f' % num,'lr': '%.8f' % lr,'e_greed':'%.3f' %self.agent.explorer.epsilon,'time': '%.3f' % (time.time() - t)})
                        t = time.time()
                    # if e % 1000 == 0:
...
if __name__ == '__main__':
    ...
    tm = TrainManager(env1,update_target_steps=200,episodes=100000,lr=6e-3,batch_size=512,memory_size=5000,replay_start_size=1024,decay_rate=0.0001,e_greed=0.35,num_decay=20,gamma=0.9,num_steps=20,min_lr=1e-8,factor=0.95,patience=100,threshold=1e-7)
    tm.train()